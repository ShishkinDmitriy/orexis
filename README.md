...
|---|---|---|
| `perception:PolledSensing` | `ag:Polling` — *reserved, not built* | the agent asks for each reading |
| `perception:ScheduledSensing` | `ag:Subscribing` | the agent sets an interval, the board keeps it |
| `perception:PushSensing` | `ag:Listening` | the board, alone |
...
The ESP32 firmware is `perception:ScheduledSensing`. Change the firmware, edit `ag:senseMode`, restart the
