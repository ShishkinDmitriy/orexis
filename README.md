...
`infra/mosquitto/Containerfile` is Debian + the stock `mosquitto` package, and it exists because
this host has mosquitto installed as an apt package — which ships an AppArmor profile bound to
`/usr/sbin/mosquitto`, and therefore confines the *container's* broker too. The profile permits
`/etc/mosquitto/*` and `conf.d/**` and nothing else, which is why the Alpine image (config under
`/mosquitto/config/`) could not read its config by *any* means, and why generated files are
mounted directly in `/etc/mosquitto` rather than a subdirectory of it. It was never musl, and it
is not the [upstream reports](https://github.com/eclipse-mosquitto/mosquitto/issues/2557) either.
Without a config mosquitto binds loopback only, so the boards never reach it.
