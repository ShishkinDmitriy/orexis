...
**The KY-015 delegates to `wokwi-dht22`, which is the wrong part.** DHT11 and DHT22 do not encode
alike: a DHT11 sends integer bytes with zero decimals, a DHT22 sends 16-bit values scaled by ten
with a signed temperature. The firmware asks for `DHT11`, so the checksum will fail and `logAir()`
will report no answer. The board is still worth having — it draws the right part with the right
pins — but making the simulation truthful needs a `chip-ky-015` implementing DHT11 framing, which
is also where temperature and humidity sliders would live.
