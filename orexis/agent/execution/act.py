...
WHAT DOES NOT COME ACROSS FROM THE PREDECESSOR: `takers_of`, which asked an agent for the
modules that carry an action out. Agent 2.0 loads no modules, so the question has nobody to
put it to; it returns with whatever asks it (an-agent-is-four-things).
"""
