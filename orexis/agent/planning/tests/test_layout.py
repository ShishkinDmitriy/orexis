...
def test_a_module_named_for_an_act_exports_that_act_and_nothing_else():
    ...
    """
    import ast
    package = ROOT / "orexis" / "agent" / "planning"
...
def test_outside_the_package_only_the_planner_is_imported():
    """PUBLIC MEANS PUBLIC TO THE PACKAGE. Outside `orexis/agent/planning/`, the one name the
    ...
    import ast
    package = ROOT / "orexis" / "agent" / "planning"
    for path in sorted((ROOT / "orexis").rglob("*.py")):
        if package in path.parents:
            continue
        for node in ast.walk(ast.parse(path.read_text())):
            if isinstance(node, ast.ImportFrom) and (node.module or "").startswith("orexis.agent.planning"):
                names = {a.name for a in node.names}
                assert node.module in ("orexis.agent.planning", "orexis.agent.planning.planner") \
                    and names == {"Planner"}, f"{path.relative_to(ROOT)} imports {names} from {node.module}"
            if isinstance(node, ast.Import):
                for a in node.names:
                    assert not a.name.startswith("orexis.agent.planning"), \
