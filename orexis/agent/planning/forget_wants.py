...
from orexis.agent.store import NAMESPACES, bind, instant, rows
...
def mark(store, uri: str, state: str) -> None:
    ...
    **`orexis:Done` IS TERMINAL and nothing moves a want out of it.** Stated as an invariant
    ...
    search marks a want already reached `Done` and answers with no plan, and the caller
...
def forget_wants(store) -> list[str]:
    ...
    ON THE MIND'S PASS, because a want is planning's and this is planning's clock. The
