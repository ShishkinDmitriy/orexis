...
def test_the_rows_presence_is_the_openness_and_no_rule_asks_the_clock(make):
    ...
    def buying(at=None) -> bool:
        ...
        return any(row.action == ACQUIRING for row in find_steps(fern.beliefs, fern.desires.abouts(fern.me.uri), fern.me.uri, picks_graph(fern.id), graphs=fern.beliefs.graphs_of(*FORESEEN, at=at or clock.now())))
