...
def test_the_menu_is_derived_from_the_graph(make):
    ...
    rows = Affordances(st.query_at, FERN, desires_build(st, "fern"), beliefs_graph("fern")).find_all()
...
def test_the_dealers_menu_gained_its_lever(make):
    ...
    rows = Affordances(st.query_at, "http://example.org/orexis/world/simulation#supplier", desires_build(st, "supplier"), beliefs_graph("supplier")).find_all()
...
def test_two_denominations_make_two_rows_and_never_four(make):
    ...
    open_round_for(st, "fern")
    acquire = [r for r in Affordances(st.query_at, FERN, desires_build(st, "fern"), beliefs_graph("fern")).find_all()
...
def test_a_duty_is_on_the_menu_and_a_stake_never_reaches_for_it(make):
    ...
    rows = Affordances(supplier.beliefs.query_at, supplier.me.uri, supplier.desires, beliefs_graph(supplier.id)).find_all()
...
def test_a_buyer_honours_nothing(make):
    ...
    fern = make("fern")
    assert all(r.is_own for r in Affordances(fern.beliefs.query_at, fern.me.uri, fern.desires, beliefs_graph(fern.id)).find_all())
