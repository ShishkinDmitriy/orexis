...
def _pursued(agent):
    ...
    return next(d for d in agent.wants() if d.uri == COMFORT or d.desire == COMFORT)
...
def test_the_bed_crosses_toward_a_cold_outside_at_the_stated_rate(monkeypatch):
    ...
    crossing = judging.crossing_of(agent.beliefs.engine, COMFORT)
    ...
    soil_only = judging.crossing_of(warm.beliefs.engine, COMFORT)
