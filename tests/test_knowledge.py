...
def test_no_document_names_a_path_that_is_not_there():
    ...
    absent_on_purpose = {
        ...
        "agent/codecs",
        "agent/scalings",
        # a-layer-is-a-distribution (#451): the mind's stores left the kernel's directory for
        # `modality/`, a root distribution of their own. The record's roster names the files
        # where they were when the decision was taken, and half a dozen older records narrate
        # the kernel vocabulary, the store, the belief reader and the two mental-state modules
        # at their old addresses — "was", "until #451", "at the time", every one.
        "agent/store.py",
        "agent/beliefs.py",
        "agent/graphs.py",
        "agent/desire.py",
...
def test_a_dictionary_term_is_a_declared_one():
    ...
    from assembly import loader
    from modality.store import NAMESPACES
