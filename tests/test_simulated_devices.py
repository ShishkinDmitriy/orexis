...
def test_the_simulated_world_derives_what_a_wired_one_does():
    ...
    #  `Reflex` and `Keeping` were in this difference and are not capabilities any more —
    #  deciding and committing are the kernel's, granted by nothing. The stake still buys the
    #  simulated fern two grants the wired one lacks; it no longer buys two that everyone has.
    assert simulated - wired == {"Bidding", "Deducing"}, \
