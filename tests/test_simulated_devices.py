...
DEVICE = "http://example.org/orexis/device#"   # the substrate, and its stand-in
...
def test_an_initial_value_outside_the_range_is_refused():
    ...
    assert not _conforms(_mutate_simulation(f"""
        DELETE {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelInitialValue 0.45 }} }}
        INSERT {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelInitialValue 45.0 }} }}
        WHERE  {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelInitialValue 0.45 }} }}"""))


def test_a_tick_of_zero_seconds_is_refused():
    """A push device would spin its clock at zero and publish without pause."""
    assert not _conforms(_mutate_simulation(f"""
        DELETE {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelTickSeconds 3 }} }}
        INSERT {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelTickSeconds 0 }} }}
        WHERE  {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelTickSeconds 3 }} }}"""))
...
def test_a_stand_in_may_share_a_neighbours_wire_without_a_bus_of_its_own():
    ...
    rows = bindings(st.query(f"""
        SELECT ?id WHERE {{ ?s <{AG}localId> ?id ; <{DEVICE}simulatedBy> ?m .
...
def test_an_initial_value_outside_a_models_own_range_is_refused():
    ...
    assert not _conforms(_mutate_simulation(f"""
        DELETE {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelInitialValue 21.0 }} }}
        INSERT {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelInitialValue 210.0 }} }}
        WHERE  {{ GRAPH <{WORLD_GRAPH}> {{ ?m device:modelInitialValue 21.0 }} }}"""))
...
def test_a_temperature_is_not_refused_for_not_being_a_fraction():
    ...
    st = genesis_store(world="simulation")
    rows = bindings(st.query(f"""
        SELECT ?initial WHERE {{ <http://example.org/orexis/world/simulation#air_temp_fern> <{DEVICE}simulatedBy> ?m .
                                 ?m <{DEVICE}modelInitialValue> ?initial }}"""))
...
def test_a_generated_stand_in_runs_at_the_worlds_pace():
    """device:timeScale rides into every stand-in's environment, and the physics arrive per
...
def test_the_pot_is_the_only_statement_of_its_own_drying():
    ...
    models = ratified.rows(ds, f"""SELECT ?m WHERE {{
        ?m a <{DEVICE}DeviceModel> . ?m <{DEVICE}modelLosesPerDay> ?v }}""")
...
def test_a_model_stating_its_own_drying_overrides_the_pot():
    ...
    ds = ratified.dataset("simulation")
    ds.update(f"""INSERT {{ GRAPH <{WORLD_GRAPH}> {{ ?m <{DEVICE}modelLosesPerDay> 0.5 }} }}
WHERE {{ GRAPH <{WORLD_GRAPH}> {{
    ?s <{AG}localId> "moisture_sensor_fern" ; <{DEVICE}simulatedBy> ?m }} }}""")
...
def test_the_meddler_is_its_own_service_with_its_own_credential():
    ...
    image, listed in compose only because the world states device:strayDoseMeanDays."""
