...
from orexis_agent_deliberation import derive_wants as judging
...
def test_a_claim_arriving_writes_a_debt_and_a_prediction_and_the_derivation_mints_the_want(monkeypatch):
    ...
    agent, ledger, desire = _host(monkeypatch)
    ...
    [want] = find_wants(agent.beliefs, desire=desire)          # every desire derives; this one's
    assert want.desire == desire and want.about == (debt,)
    assert want.uri.startswith(desire + ".pursued.obligation.")
    ...
    assert abs(judging.crossing_of(agent.beliefs.engine, desire).timestamp() - deadline) < 1.0
...
def test_presenting_makes_the_roads_want_pursuable_and_paying_withdraws_its_ground(monkeypatch):
    ...
    assert judging.crossing_of(agent.beliefs.engine, desire) is None, \
...
def test_a_second_claim_is_a_second_want_and_the_first_stands(monkeypatch):
    ...
    assert {w.uri for w in find_wants(agent.beliefs, desire=desire)} == {first.uri, second.uri}
...
def test_what_was_foreseen_has_arrived_when_the_holder_asks_before_the_lapse(monkeypatch):
    ...
    agent, ledger, desire = _host(monkeypatch, stock=3.0)
    deadline = time.time() + HOUR
