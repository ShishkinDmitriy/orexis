"""Compartments: which predicates are joined by what the shipped rules actually do (#565).
...
not: every world's vocabulary is ONE compartment. The measurement is the point, and it is why
one cone per compartment has nothing yet to split.
"""
...
def _parts(store):
    return R.compartments(R.actions_of(store.query), R.rule_edges())


@pytest.mark.parametrize("world", WORLDS)
def test_every_shipped_world_is_one_compartment(world):
    """The measured claim, before any mechanism rests on it: nothing a shipped world does
    separates its vocabulary. Ninety-odd predicates, one compartment — whether the derivations
    ...
    actions_alone = R.compartments(R.actions_of(store.query))
...
def test_every_shipped_want_falls_inside_one_compartment(world, monkeypatch):
    """The claim as each WANT sees it: a want's view — what it reads, plus what the actions
    relevant to it read and write — lies inside one compartment, so its plan is one cone. True
    for every want every shipped agent holds, and true trivially while the vocabulary is one
    compartment; the test below is what keeps this one honest."""
    ...
    monkeypatch.setenv("INFLUX_BUCKET", "test-compartments")
    monkeypatch.setenv("INFLUX_TOKEN", "test-token-compartments")
    ...
    for agent_id in agents:
        ...
        for want in agent.pursuing():
            ...
            assert R.spans(view, parts) == 1, \
                f"{agent_id}'s {want.uri.rsplit('#', 1)[-1]} spans several compartments"
            planner.reset()
...
def test_a_vocabulary_nothing_joins_falls_apart():
    ...
    if this computation returned one compartment for everything, so it is asked of two actions
    that share no predicate at all: two compartments, and a view of one of them spans one."""
    actions = {"urn:a": (frozenset({"urn:reads:x"}), frozenset({"urn:writes:x"})),
               "urn:b": (frozenset({"urn:reads:y"}), frozenset({"urn:writes:y"}))}
    parts = R.compartments(actions)
    assert len(parts) == 2
...
def test_one_action_across_two_halves_joins_them():
    ...
    The greenhouse's case, in miniature — two vocabularies are two compartments until one lever
    ...
    assert len(R.compartments(actions)) == 2
    coupled = dict(actions, **{"urn:heater": (frozenset({"urn:climate"}), frozenset({"urn:soil"}))})
    assert len(R.compartments(coupled)) == 1, "one lever across both makes one compartment"
...
def test_an_unreadable_lever_joins_everything():
    ...
    a compartment wrongly split would let two plans contradict each other."""
    ...
    assert len(R.compartments(with_blind)) == 1
    assert R.spans(R.ANYTHING, R.compartments(actions)) == 2, "and an unreadable view spans all of them"
