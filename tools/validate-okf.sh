#!/usr/bin/env bash
# OKF Bundle Validator v0.1 — VENDORED from the okf-open-knowledge-format skill.
#
# Here rather than invoked from ~/.claude/skills/ because it is one of this project's gates, and a
# gate that lives in one person's home directory cannot be run by a fresh clone or by CI. The
# published `okflint` is not a drop-in: it wants an okf-base.yaml manifest this bundle does not
# carry.
#
# If the skill's copy gains checks, re-copy this one.
