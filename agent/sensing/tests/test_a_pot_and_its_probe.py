...
replaces the first and says where it surprised.
...
def _reading(store, snapshots, probe, value, minutes=0):
    """Bytes arrive: received, then what was predicted is asked, then the stretches are rewritten,
    and every graph written is revised — the order a container would keep."""
...
def test_a_second_reading_replaces_the_first_and_says_where_it_surprised(pot, snapshots):
    store, probe = pot
    _, first, _ = _reading(store, snapshots, probe, 0.25)
    graph, second, said = _reading(store, snapshots, probe, 0.09, minutes=16)
    assert said is not None and "under the floor of zz.operating" in said, said
