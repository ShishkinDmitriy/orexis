...
- `:ontology` — the shared **T-Box**, merged from every package's `ontology.ttl`
  (the kernel's own `agent_old/ontology.ttl` and every `packages/<family>/<name>/`): classes and properties (World,
...
`packages/orexis-agent-progression/upkeep.py` holds the clock and calls `optimize()` when the ratio it is held to says so
— the clock is progression's, and so is the store ENGINE the `optimize()` runs on
(`packages/orexis-agent-progression/store.py`), while the belief base as a MODALITY — the
reader, the picks, what a belief means — is deliberation's
(`packages/orexis-agent-deliberation/beliefs.py`): since
[a-layer-is-a-package-and-need-loads-it](/decisions/a-layer-is-a-package-and-need-loads-it.md)
the engine sits in the lowest layer that persists anything and the search reaches DOWN to it,
never the reverse; upkeep compacts the store it is handed and never reads a belief in it.
