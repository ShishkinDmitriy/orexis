...
|---|---|
| `perception:polls` a sensor whose `perception:senseMode` is `perception:ScheduledSensing` | `perception:Subscribing` |
| `perception:polls` a sensor whose `perception:senseMode` is `perception:PushSensing` | `perception:Listening` |
...
**Sense mode is honoured rather than bypassed.** `perception:ScheduledSensing` means the stand-in keeps the
interval its agent gives it over the retained command, exactly as a deep-sleeping board does;
`perception:PushSensing` means it keeps its own clock and takes no orders. So the simulation exercises the
