...
description: >-
  ...
  compartments cannot contradict, because no action of one writes a fact the other reads, which
  ...
  compartment, so nothing yet splits.
---

# What it is

Take every [action](/domain/action.md) and every derivation, and join the predicates each one
reads or writes. What falls out is a partition of the vocabulary: within a compartment, some
lever couples the words; across compartments, nothing does. The word is a hull's: flooding one
compartment does not flood the next, because nothing passes between them.

**Not "component", and not "scope".** Component is what this repo calls a package, and the graph
theory it comes from is not the reader's. Scope is taken twice already — what a commitment's
token permits, and the axis an `orexis:Always` want sits on — and a third meaning is how one
concept becomes three.
...
different compartments **cannot contradict**: no action serving one writes a fact the other reads,
...
- A greenhouse's water words and climate words look like two vocabularies until a heater dries
  the soil. One lever across both makes one compartment.
- Two vans in one courier vocabulary look like one until you notice nothing they do touches the
  same van.

The second names the limit. A compartment here is a set of PREDICATES, so it separates a
vocabulary and never two instances of one. Two vans are two compartments only over VARIABLES, a
...
might touch any predicate cannot be proven not to, and a compartment wrongly split would let two
plans contradict each other.

# What it measures today: nothing splits

Every shipped world is ONE compartment, around ninety predicates, whether the derivations are
...
shape. Every want every shipped agent holds falls inside that one compartment.

So the mechanism this concept exists for — one cone per compartment, plans concatenated — has
