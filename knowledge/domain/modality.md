...
progression (`packages/orexis-agent-progression/store.py`, `graphs.py`, `ontology.py`) with
the intention modality, because progression is the lowest layer that persists anything;
`Beliefs` and `Desires` sit in deliberation (`packages/orexis-agent-deliberation/beliefs.py`,
