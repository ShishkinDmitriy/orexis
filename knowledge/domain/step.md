...
A **step** is an [act](/domain/act.md) at a position in a plan: depth, order, and what the
search predicted — the urgency the world would have after it, and whether the want would be
met there. `Plan.steps` holds them; the [imaginarium](/domain/imaginarium.md) holds the world
each one reached, for the life of one pass.

# Only the head is committed

The plan is re-derived every pass because the world moves, so a committed tail would be a
promise about a future nobody has seen. [Execution](/domain/execution.md) takes the first
