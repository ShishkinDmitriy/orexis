...
| 2026-09-16 | `a-rule-does-not-say-which-world-it-reads` | 0.31 s / 50 forks (from 0.48) | 0.18 s (from 0.26) | #666: 48 `GRAPH $state` wrappers out of 11 files, and the door told which world instead. FASTER, and by deletion rather than by anything clever — five doubled clauses became one each (a round asked of belief and of the imagined world alike; four debt discharges), the kernel's class check lost a UNION, and a pattern asked twice is asked once. `world/loner`'s dry pot 1.40 -> 1.32 s, fork counts unchanged everywhere, best of five on the bench. REFUSED with it (PR #670): holding a node as a DIFF rather than a copy, which needs precedence, which needs a SPARQL rewriter of our own since rdflib will not re-emit a CONSTRUCT — measured at 1.08-1.22x a read and no fork at all, winning only past a few thousand triples of state, which is above every shipped world. Measured on the way and kept: engine reads are about 15% of a pass and the fork 0.1%, so the fork's copy is not where a pass spends its time |
