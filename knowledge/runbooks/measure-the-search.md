...
| 2026-09-08 | `a-causal-component-is-measured-before-it-is-a-mechanism` | — | — | causal components from relevance's tables (#565 item 1): predicates joined wherever one action or derivation reads or writes both. Measured on every shipped world — loner, simulation, courier, hanoi — ONE component of about ninety predicates, with the derivations counted or the actions taken alone, and every want of every shipped agent inside it. So one cone per component has nothing to split and is not built. The actions alone already join the market's plumbing to sensing's readings, because every shipped effect predicts a reading of the same shape; and a component of PREDICATES cannot separate two vans, which needs a variable, a subject and a predicate together |
