...
| 2026-09-15 | `a-fork-copies-the-half-a-pass-can-touch` | 0.76 s / 50 forks (unchanged) | — | #662: a pass sets aside the readings its view names nothing of, so a fork copies what it could touch. The bench is hanoi padded with facts no action writes — the fork goes 1.0 / 8.5 / 34.8 / 57.9 % of the pass at 0 / 100 / 1,000 / 5,000 padded facts to 1.1 / 2.8 / 2.6 / 1.6 %, with the mutable half at 3 quads in every row. On `world/loner` padded with 5,000 readings of another property, 25,008 mutable quads become 8; padded with 5,000 of the property the want is ABOUT it stays 25,008, which is the predicate-level limit the scopes row below already measured — separating two pots needs a variable, as separating two vans does. Every shipped world plans what it planned. REFUSED on measuring: a node's graph holding only the places its path changed (hanoi's Move walks the tower and the courier's Pick reads every parcel, inside `GRAPH $state`), and a reading kept only for the subject the agent acts for (the greenhouse's vent reads the OUTSIDE's air, and the cold night stopped being refused) |
