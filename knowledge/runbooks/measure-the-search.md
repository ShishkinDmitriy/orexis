...
| 2026-09-07 | `an-effect-declares-two-bounds` | — | — | an effect declares two bounds (#556): the three rules that predict a reading bind its ends from the tolerance they read in the beliefs, a standing width compounds along a chain (the nearly-empty butt's two doses predict 0.057–0.090 then 0.073–0.140), the signature folds the pair into one fact, the keeper holds the world to the step's own interval and takes no tolerance from an actor. Step and fork counts unchanged: hanoi 7 steps / 50 forks, courier 8 / 78, the plant 2 / 4. Three engine-side traps found and pinned: a decimal product past eighteen fractional digits binds nothing (the movement is rounded to six places), `a / b * c` is read as `a / (b * c)` (parenthesised), and an apostrophe in a rule's `#` comment opened a string for the premises reader (comments skipped now) |
