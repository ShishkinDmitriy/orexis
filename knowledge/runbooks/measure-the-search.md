...
| 2026-09-14 | `every-want-sourced-at-a-time-is-a-graph-with-a-period` | — | — | #645: a round, a claim, a cooling row, a debt and a child past their ends are invisible to every reader and gone after one `upkeep.sweep`; a debt lapsing unserved leaves `market:lapsedAt` in the record and no want; a reborn host finds what lapsed while it was down and sweeps it at boot |
| 2026-09-14 | `the-reviser-wakes-on-a-surprise` | — | — | #632: ten readings inside the loner pot's expected bands leave no mark, one below leaves one and the pass writes `deliberation:surprise`; a noisy probe near the floor is expected inside or below, so a reading that crosses the floor wakes nothing and one above does; with the ladder dropped a reading marks as before |
| 2026-09-14 | `the-keeper-tells-sensing-the-intended-branch` | — | — | #639: a watch on a reading holds no shape — the keeper tells `orexis:predicted` the band and the instants, sensing writes the band into the predictions from the landing and compares once at arrival; the loner's two-dose plan advances on a reading in the band, drops on one outside past the landing, and stays open on one before it |
| 2026-09-14 | `the-search-reads-predictions` | — | — | #643: the search overlays the prediction holding at a node's instant and runs no drift; a crossing is the start of the earliest prediction at which the root reads unmet, so its resolution is the package's ladder — the loner at 0.12 (below a day out) is foreseen five hours out where the rule said sixteen, the simulation fern at 0.47 an hour out where the rule said four, the supplier owing by the second hour an hour out; nothing dries after a step inside a pass, the record's seam. Hanoi and the courier have no drift and are untouched |
...
| 2026-09-16 | `the-mutable-slice-was-narrowed-and-not-taken` | 0.76 s / 50 forks (unchanged) | — | WHERE A PASS ACTUALLY SPENDS ITS TIME, measured because #662 proposed shrinking what a fork copies. On hanoi: engine reads 111 ms over 670 queries against 8 ms of forking over 52 updates — **14 to 1**, and reads are about 15% of the 0.76 s pass. On `world/loner` with 2,000 padded subjects the pass is 6.11 s, of which `_begin` is 4.94 s (81%): the canonicaliser 3.25 s, relevance 1.47 s, the invariant signature 0.95 s, the rdflib flatten 0.88 s — and forking 7 ms (0.1%). THE CANONICALISER IS QUADRATIC in keyed nodes, scanning every triple per node to build its key: 17 / 64 / 256 / 1,151 / 6,331 ms at 250 / 500 / 1,000 / 2,000 / 4,000 readings, x4 for each doubling. The narrowing itself works — a padded 3-disk solve goes 1.0 / 8.5 / 34.8 / 57.9 % of the pass spent forking at 0 / 100 / 1,000 / 5,000 padded facts to 1.1 / 2.8 / 2.6 / 1.6 %, mutable half 3 quads in every row, every shipped world planning what it planned — and was not taken: it wins on padded worlds nobody has composed and adds an assumption on the ones that exist. REFUSED on measuring: a node's graph holding only the places its path changed (hanoi's Move walks the tower and the courier's Pick reads every parcel, inside `GRAPH $state`), and a reading kept only for the subject the agent acts for (the greenhouse's vent reads the OUTSIDE's air, and the cold night stopped being refused) |
| 2026-09-16 | `a-rule-does-not-say-which-world-it-reads` | 0.31 s / 50 forks (unchanged) | 0.18 s (unchanged) | #666: 48 `GRAPH $state` wrappers out of 11 files, and the door told which world instead. A WASH — the sides alternated in one session, best of five each, land inside each other's noise (3 disks 306/321 before against 318/304 after; the loner's dry pot 1.31/1.35 against 1.35/1.31), fork counts unchanged everywhere. It was first recorded as FASTER, 0.48 -> 0.31 s, and that was the BENCH DRIFTING between two separate invocations rather than the change doing anything: ALTERNATE THE SIDES WITHIN ONE SESSION or a row here says more about the Pi than about the commit. Under cProfile the engine's own time rose 132 -> 184 ms over the identical 669 queries, a pattern once scoped to one small named graph now reading the merged default graph; the deletions pay for it — five doubled clauses became one each, and the kernel's class check lost a UNION — and the net does not move. REFUSED with it (PR #670): holding a node as a DIFF rather than a copy, which needs precedence, which needs a SPARQL rewriter of our own since rdflib will not re-emit a CONSTRUCT — measured at 1.08-1.22x a read and no fork at all, winning only past a few thousand triples of state, which is above every shipped world. Measured on the way and kept: engine reads are about 15% of a pass and the fork 0.1%, so the fork's copy is not where a pass spends its time |
