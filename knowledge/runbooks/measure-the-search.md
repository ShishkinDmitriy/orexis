...
| 2026-09-11 | `the-door-honours-an-interval` | — | — | a graph says when it is worth believing and the door drops what is outside its interval (`validity-belongs-to-the-named-graph`, #589's first half). The filter is where the clock is read and the only place: 0.9 us per `public_graphs()` call with no interval stated — the fast path every shipped world takes, since none states one — and 1.9 us with one, against the milliseconds of the query it precedes. The TABLE is remembered rather than a filtered list, because which graphs are valid depends on when it is asked and the table does not, so a memo keyed by an instant would miss on every call |
