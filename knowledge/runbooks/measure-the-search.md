...
| 2026-09-10 | `a-node-is-a-world-and-an-instant` | 0.50 s / 50 forks (unchanged) | 0.74 s / 14 forks (unchanged) | identity is world-and-when (#587): a node's key is its diff AND the path's own `orexis:landsAfter` summed, so cycle detection stops collapsing two instants into one world. EVERY SHIPPED WORLD IS UNCHANGED, forks and steps both — hanoi 14 and 50, the courier's corner delivery 71, the greenhouse 128 in both knob regimes, both plants 2 — because doubling back inside a budget needs an action that declares a landing AND returns to a pose, and no shipped world has one. THE COST WHERE ONE DOES, measured by giving a hanoi move a minute: two disks 14 forks to 17, three disks 50 to the whole 128-world budget, still three and seven moves — and at that world's own budget of 64 the timed three-disk solve now answers `exhausted` at 64 forks where it used to solve. A step that MOVED NOTHING is exempt and had to be: with the instant alone deciding, the market host owing water it does not hold served from a barrel too low instead of planning the refill, because a dry serve predicts nothing and had stopped colliding with its parent. That is cycle detection's grip on a world with a clock, and it is what argues the GRAIN two instants are told apart at belongs to the drift (#592) rather than beside it: until a rule reads the clock or a reading changes with time, a finer instant buys distinctions with no content |
