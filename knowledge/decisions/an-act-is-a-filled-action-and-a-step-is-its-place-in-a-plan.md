...
and want (`orexis:by`, `orexis:through`, `orexis:pursues`) and not the size. A
...
and what that is about, quantity, whom for, and a **window** — not before, not after. What an intention commits to (`orexis:by`
...
1. `orexis:Act` and the kernel dataclass; an affordance row becomes an act when the search sizes
...
2. An intention commits to an act — `orexis:by` names the act node, which `orexis:fills` the action
   and carries lever, quantity and window; the ledger's `orexis:through` became the act's; old
...
3. A commitment promises an act; `Claim` carries the host's Serving act; `exp` is the act's
   ...
   self-dose commits to a Dosing act windowed by its landing and seeing time; `orexis:deadlineAt`
   is gone and the watch's deadline IS the act's `orexis:notAfter`, which the bidder's give-up
...
- **A step's predicted world is trace only.** `orexis:wouldReach` on a candidate is a number; the
