...
description: >-
  ...
  property, quantity, whom for, and when — and it had no name: an affordance row carried
...
row is action + lever + property, unsized. The planner's `_Node.taken` is a tuple of rows plus
the sizing it asked the taker for. An [intention](/domain/intention.md) records action, lever
and property (`ag:by`, `ag:through`, `ssn:forProperty`) and not the size. A
...
**An act is a filled action, and it is execution's word.** Action, lever, property, quantity,
whom for, and a **window** — not before, not after. What an intention commits to (`ag:by`
...
1. `ag:Act` and the kernel dataclass; an affordance row becomes an act when the search sizes
   it; `Plan.steps` hold steps whose acts are those; `take(act, …)` (#369).
2. An intention commits to an act — `ag:by` names the act node, which carries action, lever,
   property, quantity and window; the ledger's `ag:through` and `ssn:forProperty` become the
   act's; old ledgers migrate (#370).
