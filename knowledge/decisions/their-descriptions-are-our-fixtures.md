...
spelling. `sh:in` now says which spellings can be *read*, where `vocabulary/dht11/` shows the one
...
- **The frequency figure is corroborated.** Their datasheet says 2 seconds, which is what
  `vocabulary/dht11/` declares for the KY-015 — a number nobody here measured, confirmed by the
