...
sensors share machinery — is unfounded. Driven through the real module with two `perception:ScheduledSampling`
...
An agent with one `perception:ScheduledSampling` and one `perception:PushReporting` sensor derives both `perception:Subscribing` and
