...
sensors share machinery — is unfounded. Driven through the real module with two `perception:ScheduledSensing`
...
An agent with one `perception:ScheduledSensing` and one `perception:PushSensing` sensor derives both `perception:Subscribing` and
