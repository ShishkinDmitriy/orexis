...
superseded-by: a-package-is-its-name
...
```
packages/core/orexis            packages/orexis-capability-market
packages/orexis-bus-onewire           packages/orexis-transport-mqtt
packages/orexis-part-dht11            packages/orexis-codec-json
packages/orexis-plant-zamioculcas     packages/orexis-scaling-identity
packages/orexis-tool-wokwi
```

A package holds whichever of `ontology.ttl`, `shapes.ttl`, `rules.ru`, `review.rq` and Python it
wants. Every one optional, **and an omission is a statement**: `packages/orexis-part-esp32/` is an
ontology and nothing else because a board has no behaviour a runtime could load, and
`packages/orexis-capability-market/` has all of it. Neither is more of a package than the other.
...
would file apart the two packages most worth reading together. `packages/orexis-part-dht11/` and
`packages/orexis-plant-zamioculcas/` are structurally identical: a model described once whose units a
...
(`orexis_capability_market`), and a contract that can name a family in one line. Capabilities
...
**The layout no longer shows which Python a runtime loads.** `packages/orexis-capability-market/` and
`packages/orexis-part-dht11/` look identical, and the reason the old split existed was exactly that
