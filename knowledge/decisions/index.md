...
* [matching-is-the-word](/decisions/matching-is-the-word.md) - Three words were in circulation for one concept — matching, rule and format — all three in hosting.py alone. Matching wins because it is accurate about what we model: allocation and prices, not how bidding proceeds. Format over-claims and means two incompatible things in the wild (a "Dutch auction" is descending-open to a theorist, uniform-price-sealed to a banker); rule collides with rules.ru. No identifier changed and no behaviour moved.
