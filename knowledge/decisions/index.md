...
* [matching-is-a-capability](/decisions/matching-is-a-capability.md) - Turning a lot and a set of bids into an allocation with prices left agent/auction.py and became ag:MatchingCapability, pay-as-bid implemented and uniform-price declared beside it; the host states how it matches and the capability is derived from that. It is public and announced in the offer, because a bidder cannot bid well against terms it does not know. Saying it required fixing a conflation first: a market is the standing structure, an auction is the process that condenses inside it.
* [matching-is-the-word](/decisions/matching-is-the-word.md) - Three words were in circulation for one concept — matching, rule and format — all three in hosting.py alone. Matching wins because it is accurate about what we model: the allocation rule and the payment rule, not how bidding proceeds. Format over-claims and means two incompatible things in the wild (a "Dutch auction" is descending-open to a theorist, uniform-price-sealed to a banker); rule collides with rules.ru. No identifier changed and no behaviour moved.
