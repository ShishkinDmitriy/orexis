...
which between them decide most arguments. After that
[capability-packages](/decisions/capability-packages.md) for how a package is found,
[a-desire-is-a-shape](/decisions/a-desire-is-a-shape.md) for what an agent wants, and
[a-plan-is-a-path-of-graph-diffs](/decisions/a-plan-is-a-path-of-graph-diffs.md) for what it does
about it. **Going to change something?** Find the section it lives in below and read that section
whole — the seams are what you are checking for.
...
* [a-want-met-by-absence](/decisions/a-want-met-by-absence.md) - A desire may say unmetWhen — the avoided pattern, kernel-judged, never both tests; the capability built first dissolved, and the law prunes at expansion.
* [shall-must-and-may-are-not-strengths-of-one-scale](/decisions/shall-must-and-may-are-not-strengths-of-one-scale.md) - SHALL is the desire, deontic MUST the obligation, unbreakable MUST the shapes, MAY the plan library — different logics, never one scale.
...
* [the-effect-is-one-declaration-and-the-tolerance-is-a-pick](/decisions/the-effect-is-one-declaration-and-the-tolerance-is-a-pick.md) - The step's predicted diff is what the world is held to; the actor adds only a tolerance, a revisable pick.
...
* [a-remembered-plan-is-a-method-on-the-want](/decisions/a-remembered-plan-is-a-method-on-the-want.md) - A plan that worked is kept on its want, keyed by the world's signature and verified by feedback; re-simulation before reuse refused.
...
* [a-predicted-number-is-an-interval](/decisions/a-predicted-number-is-an-interval.md) - A number in a possible world has two ends; met is the interval inside the region, and a chain too wide has a look in it.
...
* [an-injected-service-is-reached-by-term](/decisions/an-injected-service-is-reached-by-term.md) - The design for a package offering another a store or a service, keyed by URI. Recorded, not built; #311 is the trigger.
...
* [a-guard-that-asks-the-filesystem-asks-about-somebodys-machine](/decisions/a-guard-that-asks-the-filesystem-asks-about-somebodys-machine.md) - Tracked, ignored and missing are three answers; asking `exists()` collapses them and hides a deleted world behind a leftover.
* [a-deferred-import-is-code-no-test-runs](/decisions/a-deferred-import-is-code-no-test-runs.md) - An import inside a function runs only when that function does, so one crash-looped every agent while three gates stayed green.
* [a-generated-credential-is-not-a-file-we-keep](/decisions/a-generated-credential-is-not-a-file-we-keep.md) - A directory pattern cannot prune a directory holding tracked files, so the ignore rule watched a leak it could not stop.
* [a-shape-is-named-for-what-it-checks](/decisions/a-shape-is-named-for-what-it-checks.md) - A gate prints a message and a reader lands on a name, so a name for a check that left is a signpost to nothing.
