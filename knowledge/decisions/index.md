...
* [who-put-the-fact-there](/decisions/who-put-the-fact-there.md) - Public knowledge is five graphs split by who put the fact there: asserted, derived and entailed, for the vocabulary and for the world. An ordinary query still reads all five because they are its default graph — and a SELECT that names one reads only part, silently. Both engines now share one derivation instead of computing separately, which is how a live regression in the supplier's signing keys was found.
* [a-version-is-a-snapshot](/decisions/a-version-is-a-snapshot.md) - The append-only chain the vocabulary promised for a year and never kept. A version is a whole snapshot, so retraction needs no vocabulary — a device that moved is absent from the later one — and who introduced a triple stays computable. The head is derived rather than stated, so ratifying only appends; a content hash over raw bytes catches an amendment that forgot to ratify, which is what finally lets `world_version` differ.
