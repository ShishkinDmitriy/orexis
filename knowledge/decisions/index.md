...
* [an-auction-format-is-a-capability](/decisions/an-auction-format-is-a-capability.md) - The allocation rule left agent/auction.py and became ag:MatchingCapability, pay-as-bid implemented and uniform-price declared beside it; the host states which it runs and the capability is derived from that. The rule is public and announced in the offer, because a bidder cannot bid well against terms it does not know. Saying it required fixing a conflation first: a market is the standing structure, an auction is the process that condenses inside it.
