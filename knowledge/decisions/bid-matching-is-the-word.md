...
allocation with prices, then its own package and now `packages/orexis-capability-market/matching.py`. It was called **matching** (the directory, the
...
- **The bidding procedure is not a family.** Naming the axis is not building a slot for it. If
  ...
  in `packages/orexis-capability-market`, and Dutch would join *it* while still matching by pay-as-bid.
