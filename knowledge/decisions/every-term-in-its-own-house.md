...
- **The simulated device model — seven terms** *(eleven since the scenario grew: the physics
  went time-based and the weather arrived)*. (AMENDED: every term in this paragraph is `device:` now, in
  `packages/part/device/` — see [the-substrate-is-not-the-minds](/decisions/the-substrate-is-not-the-minds.md).)
