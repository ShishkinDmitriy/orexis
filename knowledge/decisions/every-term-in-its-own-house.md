...
`domains/climate` is the one that matters beyond tidiness. AGENTS.md opens by saying the v1
...
source tree for a full IRI in `orexis:` whose local name the kernel's `agent_old/ontology.ttl` does not
declare. It is
...
Answering the question this sweep was supposed to answer. The kernel — `agent_old/ontology.ttl`, then
...
- **The kernel vocabulary (`packages/orexis-agent-progression/ontology.py` since #452) still overstates itself**, by eleven terms rather than a hundred and
