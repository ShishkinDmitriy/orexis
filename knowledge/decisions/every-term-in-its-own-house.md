...
`modality/ontology.py` (then `agent/ontology.py`) opens by saying everything in the kernel is true of *every* capability.
...
- **`modality/ontology.py` (`agent/ontology.py` until #451) still overstates itself**, by eleven terms rather than a hundred and
...
- **A package's `terms.py` is the one place its namespace is written.** Two `beliefs.py`
  ...
  divergence.
- **Namespace constants for cross-tree references** sit in `modality/ontology.py` beside the
