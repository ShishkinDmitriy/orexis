---
type: Decision
title: A qualitative state is a class the domain declares, and the core plans on class triples
status: accepted
timestamp: 2026-09-07T18:00:00Z
description: >-
  A range, a cell and an interval are not concepts of the core. The domain declares what a
  reading can BE — dry, in region, wet; empty, low, full — as OWL classes with datatype
  restrictions, minted per want by the derivation that mints the want; a reading is classified
  by entailment; a step's precondition and its effect are class triples; and progression
  computes the numbers at execution, sizing the dose or the lot to reach the class the step
  promised. Refused, each after being built: a partition the kernel reads off the shapes and
  selects (#573), a predicted number carried in the core as an interval (PR #575), and the search
  sizing an act during simulation. The cost, taken knowingly: two acts each too small to cross a
  boundary are found by re-planning after the first lands, not by search.
---
...
**What a reading can be is the domain's to say, as classes.** `orexis-plant-water` declares the
families — dry, in region, wet, and the butt's empty, low, full — as OWL classes with datatype
restrictions on the value they classify. Dry is relative to the plant, and a restriction takes a
fixed literal, so the MEMBER classes are minted per want by the derivation that mints the region
want from the range the world states (`desires.ru`, on the desire modality's rebuild, so a class
keyed to a pick moves when the pick does). The shape says what must hold and the class says what
a reading is, both from one range, so there is one owner. The domain already had the word:
`water:Band` with `LOW` and `OK`, the alarm band a board keeps, stated as instances.

**A reading is classified by entailment**, the way the closure already turns `owl:hasValue` into
ground triples (`agent/inference.py`, rule 5): a datatype restriction over a number is the second
OWL construct a materialising closure can honour. For a reading it is computed, never asserted,
since the interpreter already knows the number.

**The core sees classes and nothing else.** A [step](/domain/step.md)'s
[precondition](/domain/precondition.md) is "the reading is a dry one" and its
[effect](/domain/effect.md) is "the reading is in region", plain triples. The signature states a
reading by its class; identification, cycle detection, met and the regression are triple
equality. The keeper holds the world to the class the step promised. No cell, no interval, no
partition, nothing keyed by a number.

**Progression computes the numbers.** The effect rule stays SPARQL in the package and declares
the class a dose reaches from the class it starts in; the actuator sizes the dose and the bidder
the lot at execution, from the reading in hand, to reach the promised class — what `dose_for`
...
plans on; how urgent a class is, is the measure's to say.
...
- **Where membership is computed** — at the signature, reading the T-Box's restrictions, or
  materialised into a possible world at the fork. Measured in #576.
