...
description: >-
  `ag:Means` was a class of five individuals that nothing read. A means existed to be the
  ...
  (`ag:by`), a row carries it, a trace weighs it, the packages' short-lived means
  (`market:Acquire`, `sensing:Observe`, …) go with the class, and the kernel's ontology keeps
  `ag:Action` and no individual of anything.
status: accepted
...
Five means, each typed `ag:Means`: Observe, Actuate, Acquire, Apply, Offer. `means.md` called
a means "the joint" — the one term an affordance row offered, an effect rule attached to
(`ag:effectOf`), and an intention committed to (`ag:by`), so that three parts of the design
...
[an-action-is-one-node](/decisions/an-action-is-one-node.md) folded them: one `ag:Action`
node carries the precondition, the effect and the taker. From then on every shipped means
pointed at exactly one action and every action named exactly one means — `ag:means` was a
one-to-one edge whose two ends could never differ. The sovereign asked what `ag:Means` was
for, and the grep answered: nothing reads it. No query asks `?x a ag:Means`, no shape targets
it, no code names it. It was the `ag:Mode` argument again — a term whose whole content is
...
what an intention commits to (`ag:by`), what a trace weighs (`ag:wouldTake`) and what the
...
intention could name nothing else. `ag:Means`, `ag:means` and the five individuals are
deleted; the kernel's ontology keeps `ag:Action` and names no individual of anything.
...
- **A package with two actions of one kind** has no word for the kind they share. The day one
  ...
  sub-class of `ag:Action`, not a return of `ag:Means` — and nothing in the kernel changes.
