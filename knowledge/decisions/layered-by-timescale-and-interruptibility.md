...
`packages/orexis-agent-reactive/`, `packages/orexis-agent-progression/`,
`packages/orexis-agent-deliberation/`, each importing only the layers beneath it, and
...
|---|---|---|
| reactive | the loop (`orexis_agent_reactive.loop`) — one FIFO, one daemon | every handler, every take, every timer's function, every event told through the choir. An item is milliseconds and never waits |
| progression | the scheduler (`orexis_agent_progression.scheduler`) — a heap of deadlines | nothing. It sleeps until the next deadline and ENQUEUES; a wait is an entry here that enqueues a check |
| deliberation | the worker (`orexis_agent_deliberation.reviser`) — the mind's own | the search, and only the search. Its RESULT crosses onto the loop as one item: the plan's head, committed and taken |
...
- ~~**The revision function is implicit.**~~ Named: `agent/reviser.py` — `packages/orexis-agent-deliberation/reviser.py` since #452, the deliberation worker — is the seam, and
