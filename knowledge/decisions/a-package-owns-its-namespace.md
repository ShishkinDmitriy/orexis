...
**Read rather than imported, and that is forced.** Half the capabilities import `modality.store`, so a
...
- ~~**The other four packages have not converted.**~~ **Closed, and the criterion was wrong.**
  ...
  the benefit was not legibility but a bounded kernel: the kernel vocabulary (`modality/ontology.py` since #451) claims everything in it
...
- **The kernel still decides that `ag:` is what a package gets by default.** `modality.ontology.term()`
