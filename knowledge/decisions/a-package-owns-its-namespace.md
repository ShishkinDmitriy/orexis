...
**Read rather than imported, and that is forced.** Half the capabilities import `orexis_progression_patience.store`, so a
...
- ~~**The other four packages have not converted.**~~ **Closed, and the criterion was wrong.**
  ...
  then, `packages/orexis-progression-patience/ontology.py` since #452) claims everything in it
...
- **The kernel still decides that `ag:` is what a package gets by default.** `orexis_progression_patience.ontology.term()`
