...
def _service(agent_id: str, caps: set[str], world: str) -> str:
  ...
  agent-{agent_id}:
    ...
    volumes:
      ...
      # the ratified world it belongs to. Mounted at a fixed path, so the agent is told only
      # its own id and never learns that other worlds exist.
      - ../genesis/{world}:/app/world:ro{signing}
...
#
# Infra (Influx, Grafana) lives in the repo-root docker-compose.yml — bring it up first. The
...
def generate(world: str = DEFAULT_WORLD) -> Path:
    ...
    unborn = [a for a in who if not (world_dir(world) / f"beliefs-{a}.ttl").exists()]
    if unborn:
