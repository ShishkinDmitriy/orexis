...
copy: `firmware/simulated-valve/valve.py` reimplements `agora.signing.verify_command`, because
its image holds cryptography and paho and not the agora package. Duplication that nothing checks
...
VALVE_PY = Path(__file__).resolve().parents[2] / "firmware" / "simulated-valve" / "valve.py"
...
def test_the_copy_agrees_with_the_kernel(keys):
    ...
    `agora.signing.verify_command` and the firmware's copy must answer identically, including on
...
def test_the_firmware_never_imports_agora():
    ...
    assert "import agora" not in source
    assert "from agora" not in source
