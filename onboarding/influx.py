"""orexis-influx — give each agent a bucket of its own, and a token that opens only it.

  orexis-influx society        -> a bucket and a scoped token per agent in world/society
...
def _write_credential(path: Path, bucket: str, token: str) -> None:
    ...
    path.write_text(
        f"# GENERATED by `orexis-influx` — this agent's bucket, and a token that opens only it.\n"
...
def provision(world: str, rotate: bool = False) -> None:
    ...
    with InfluxDBClient(url=url, token=_admin_token(), org=org) as client:
        ...
        for agent_id in agents:
            ...
            description = f"orexis {world}/{agent_id}"
