...
def _service(agent_id: str, caps: set[str], world: str, pace: str | None = None,
  ...
  agent-{agent_id}:
    ...
    environment:
      ...
      OREXIS_WORLD_DIR: "/app/world"{_timeline(pace, epoch)}
...
# the kernel vocabulary (packages/orexis-agent-progression/ontology.py) warns about, caught by exactly the detector built to catch it — and the
...
_PACE_Q = f"""
SELECT ?scale WHERE {{ ?w a <{OREXIS}World> ; <{SIM}timeScale> ?scale }}"""


def _timeline(pace: str | None, epoch: str | None) -> str:
    """ONE TIMELINE (the-agent-keeps-one-timeline-and-its-clock-may-run-fast): a world that
    states a pace hands it to every agent's clock exactly as it hands it to every stand-in,
    as environment — a deployment fact, never a belief — with the epoch the timeline is
    measured from, so a restarted agent lands where the world's clock stands."""
    if not pace:
        return ""
    return (f'\n      # The world\'s pace and the epoch its timeline is measured from: the agent\'s\n'
            f'      # clock runs this many of the world\'s seconds per real one, as every stand-in does.\n'
            f'      OREXIS_TIME_PACE: "{pace}"\n      OREXIS_TIME_EPOCH: "{epoch}"')


def _epoch(world: str) -> str:
    """The instant a paced world's timeline is measured from. Kept from the compose file
    already beside the world, so an unchanged world regenerates an unchanged file and the
    society's timeline survives a re-onboarding; minted once where none is."""
    import re
    ...
    if existing.exists():
        found = re.search(r'OREXIS_TIME_EPOCH: "([^"]+)"', existing.read_text())
        if found:
...
def render(world: str) -> str:
    ...
    paced = ratified.rows(ratified.dataset(world), _PACE_Q)
    pace = paced[0]["scale"] if paced else None
    epoch = _epoch(world) if pace else None
    services = _broker(world, plain, tls) + "".join(
        ...
        _service(a, caps, world, pace, epoch) for a, caps in who.items())
