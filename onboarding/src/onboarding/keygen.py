...
alone, and can do nothing else with them; `agora.signing` therefore keeps `load_private`, `sign`
...
from agora import genesis, signing


def create_keypair(name: str) -> None:
    """Write `<name>.key` and `<name>.pub` into the current world's secrets directory.

    PEM/PKCS8, because `agora.signing.load_private` reads it back that way — the format is a
