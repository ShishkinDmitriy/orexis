...
refuses to start if its own beliefs do not hold — that is `agora.validate.validate_agent`, and it
...
machinery is `agora.validate`'s, shared with the agent's self-check, because two ways to decide
...
from agora.validate import conforms, graph_from
...
def validate_world(world: str) -> bool:
    ...
    data = graph_from(st, WORLD_GRAPH, *(beliefs_graph(a) for a in everyone))
