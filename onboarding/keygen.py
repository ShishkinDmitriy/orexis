"""orexis-keygen — create one world's signing keys, once, before it is onboarded.

  orexis-keygen society
...
This is here and not in the runtime for the same reason `orexis-influx` is. An agent that could
...
_ROSTER_Q = """SELECT ?a ?id WHERE {
  ?a a <http://example.org/orexis#Agent> ; <http://example.org/orexis#localId> ?id }"""
...
def roster(world: str) -> list[tuple[str, str]]:
    ...
    `orexis:supplier` has no `_agent` suffix, and the roster that assumed one attested keys for a
...
def publish_roster(world: str, ids: list[str]) -> None:
    ...
    lines = [
        ...
        "",
        "# GENERATED by orexis-keygen — the public halves of each agent's identity keys",
    ...
    for iri, agent_id in ids:
        ...
        lines.append(f'<{iri}> orexis:signingKey "{sign_b64}" ; orexis:sealingKey "{seal_b64}" .')
