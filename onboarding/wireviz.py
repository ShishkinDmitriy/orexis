"""agora-wireviz — the wiring as a WireViz harness, from the world's own statements.

  agora-wireviz sensing        writes world/sensing/wiring.yaml

Documentation of what is actually plugged in where, generated rather than drawn. It comes from
exactly the triples `agora-validate` checks, so it cannot describe a stand that does not exist
...
def render(world: str) -> str | None:
    ...
    out = [f"# GENERATED by `agora-wireviz {world}` from that world's own statements.",
...
def draft(world: str, harness: Path) -> str:
    ...
    out = [f"# DRAFT, from {harness.name}. Not a world yet.",
           ...
           "#",
           "# Left deliberately unvalidatable. A draft that passes agora-validate is the one",
           "# nobody re-reads.",
           "",
...
def import_harness(world: str, harness: Path) -> None:
    ...
    if out.exists():
        raise SystemExit(
            f"agora-wireviz: {out} already exists. An import DRAFTS a stand; it does not "
