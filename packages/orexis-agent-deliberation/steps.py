...
def _declared(store) -> list[dict]:
    ...
    """
    return store.remember(_MEMO, lambda: sorted(
        bindings(store.query(_ACTIONS, store.graphs_of(PUBLIC))), key=lambda r: r["action"]))
