...
class Plan:
    ...
    cost: float | None = None         # what the plan spends in expectation — costs over odds (#522)
...
class Planner:
    ...
    def _search(self, desire: Desire) -> Plan:
        ...
        while opened:
            ...
            for row in self._candidates(node, desire):
                ...
                #  EVERY WAY OF TURNING OUT is its own candidate (#522) — all-outcomes
                #  determinization: the step relies on one, and its odds enter as cost.
                for rule in effects.outcomes_of(self.imaginarium, row.action):
                    step = self._step_from(node, row, desire, bound, rule)
                    if step is TOO_DEAR:
    ...
    def _step_from(self, node, row, desire: Desire, bound: float | None = None, rule: dict | None = None):
        ...
        outcome = rule.get("outcome") if rule else None
        spent = effects.cost_of(self.imaginarium, row.action, outcome=outcome, **bind)
        #  EXPECTED COST (#522): a way of turning out with odds p takes 1/p attempts on
        #  average, so relying on it costs the action's price over its likelihood. Zero odds
        #  is an outcome the search will not rely on — not simulated, like a rule that will
        #  not run.
        odds = effects.likelihood_of(self.imaginarium, rule, **bind) if rule else 1.0
        if odds <= 0.0:
            return None
        cost = node.cost + (spent or 0.0) / odds
        if bound is not None and cost > bound:
            return TOO_DEAR
        try:
            added, retracted = effects.apply(self.imaginarium, row.action, outcome=outcome, **bind)
        ...
        lands = effects.lands_after(self.imaginarium, row.action, outcome=outcome, **bind)
