"""`clusterise_actions`, one case per file, held to a snapshot of the whole store it leaves.

A case in `clusterise_actions/` is the actions an agent holds — each with the effect it
...
CASES_DIR = Path(__file__).parent / "clusterise_actions"
CASES = sorted(p for p in CASES_DIR.glob("*.trig") if "." not in p.stem)


@pytest.mark.parametrize("case", CASES, ids=[c.stem for c in CASES])
def test_clusterise_actions_leaves_the_store_as_the_snapshot_says(case, monkeypatch, request, snapshots):
    ...
    clusterise_actions(agent)
    snapshots.held_to(case, request, "clusterise_actions", before, snapshots.snapshot_of(agent.beliefs))
