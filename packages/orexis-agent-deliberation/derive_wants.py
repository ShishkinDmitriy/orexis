...
def _clusters(agent, witnesses: list) -> list[list]:
    ...
    THE SCOPES ARE READ, NEVER COMPUTED: `clusterise_actions` wrote them (clusterise-actions),
    ...
    if scopes is None:
        raise RuntimeError(f"{agent.id}: the store holds no scope graph — clusterise_actions has not run")
    groups: dict = {}
    loose = []
    for w in witnesses:
