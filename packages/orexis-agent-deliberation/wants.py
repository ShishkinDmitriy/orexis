...
def find_wants(store, at: datetime | None = None, *, uri: str = "", desire: str = "",
               derived: bool = False, limit: int = PAGE, offset: int = 0) -> list[Want]:
    ...
    THE READS TAKE NO OWNER, and that is not an oversight: one agent, one volume (rule 4), so
    the store IS the scope and a want in it is this agent's by construction. A store built
    with a whole world in it is the exception `_select` keeps, and it keeps it from the
    store's own `agent_uri` rather than from a criterion a caller could forget.
