"""`clusterise_actions`: the actions an agent holds, clustered into the scopes their effects
join, written to the store — the function that makes the scope partition data
(clusterise-actions). After the call, the store says which predicates some one action or
...
log = logging.getLogger("clusterise_actions")


def clusterise_actions(agent) -> None:
